...
// ═══════════════════════════════════════════════════════════════════════════════
{
  ...
  s.addText("The 4 Tools Claude Can Call", {
  ...
  const tools = [
    ...
    { name: "find_conflicts()",  desc: "Semantic comparison of both clause arrays",         color: NAVY_LT },
    { name: "assess_risk()",     desc: "CRITICAL / HIGH / MEDIUM / LOW classification",     color: PURPLE },
    { name: "generate_redline_brief()", desc: "Compiles final structured JSON report",       color: "065A82" },
  ];
  tools.forEach((t, i) => {
    const y = 1.6 + i * 0.82;
    s.addShape(pres.shapes.RECTANGLE, {
      x: 0.55, y, w: 3.8, h: 0.7,
      fill: { color: WHITE }, line: { color: BORDER }
    });
    s.addShape(pres.shapes.RECTANGLE, {
      x: 0.55, y, w: 0.06, h: 0.7,
      fill: { color: t.color }, line: { color: t.color }
    });
    s.addText(t.name, {
      x: 0.72, y: y + 0.06, w: 3.5, h: 0.25,
      fontSize: 9.5, fontFace: "Courier New", bold: true, color: t.color, margin: 0
    });
    s.addText(t.desc, {
      x: 0.72, y: y + 0.33, w: 3.5, h: 0.28,
  ...
  const loopSteps = [
    ...
    "Agent calls find_conflicts()",
    "Agent calls assess_risk() per conflict",
  ...
  loopSteps.forEach((step, i) => {
    const y = 1.6 + i * 0.56;
    s.addShape(pres.shapes.RECTANGLE, {
      x: 5.1, y, w: 4.3, h: 0.44,
      fill: { color: i === 5 ? PURPLE : "1E3A5F" }, line: { color: i === 5 ? PURPLE : "2D5282" }
    });
    s.addText(`${i + 1}`, {
      x: 5.1, y, w: 0.35, h: 0.44,
      fontSize: 10, fontFace: "Calibri", bold: true,
      color: i === 5 ? WHITE : "CADCFC",
      align: "center", valign: "middle", margin: 0
    });
    s.addText(step, {
      x: 5.48, y, w: 3.8, h: 0.44,
      fontSize: 10, fontFace: "Calibri",
      color: WHITE, valign: "middle", margin: 0
    });
    if (i < 5) {
      s.addShape(pres.shapes.LINE, {
        x: 5.28, y: y + 0.44, w: 0, h: 0.12,
...
// ═══════════════════════════════════════════════════════════════════════════════
{
  ...
  demoSteps.forEach((d, i) => {
    ...
    s.addShape(pres.shapes.RECTANGLE, {
      x: 1.55, y, w: 8.1, h: 0.52,
      fill: { color: "162D4A" }, line: { color: "243F60" }
    });
    s.addText(d.action, {
      x: 1.7, y, w: 7.8, h: 0.52,
...
// ── WRITE FILE ────────────────────────────────────────────────────────────────
pres.writeFile({ fileName: "/home/claude/clauseguard/ClauseGuard_Presentation.pptx" })
